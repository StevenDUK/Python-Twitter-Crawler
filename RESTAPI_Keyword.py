...
def store(tweet):
    ...
    database.REST_Keyword.insert(datajson)
...
if __name__ == '__main__':
    ...
    while datetime.datetime.now() < TimerEnd:
        ...
        # Extract those posts written in English and tagged the selected keyword
        try:
            if(iteration == 0):
                tweets = API.search(q=TRACK, count='100', lang = "en")
                iteration+=1
            else:
                tweets = API.search(q=TRACK, count='100', max_id=nextMaxId, lang = "en")
                iteration+=1
